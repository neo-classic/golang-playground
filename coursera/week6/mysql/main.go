...
func (h *Handler) List(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.DB.Query("SELECT id, title, description, updated FROM items ORDER BY id DESC")
	__err_panic(err)
	for rows.Next() {
		post := &Item{}
		err = rows.Scan(&post.Id, &post.Title, &post.Description, &post.Updated)
		__err_panic(err)
...
func main() {
	...
	dsn := "root:WSSm4eTJ4TDc6Yad@tcp(localhost:3306)/ee_app?"
	// указываем кодировку
	dsn += "&charset=utf8"
